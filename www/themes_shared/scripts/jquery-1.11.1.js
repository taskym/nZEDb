...
// Create "bubbling" focus and blur events
    ...
    function getAll(context, tag) {
        ...
        if (!found) {
            for (found = [], elems = context.childNodes || context; (elem = elems[i]) != null;
                    i++) {
...
// Mark scripts as having already been evaluated
    ...
    jQuery.fn.extend({
        ...
        domManip: function (args, callback) {
            ...
            if (isFunction ||
                    ( l > 1 && typeof value === "string" && !support.checkClone &&
                            rchecked.test(value) )) {
                            ...
                            if (rscriptType.test(node.type || "") &&
                                ...
                                if (node.src) {
                                    ...
                                    jQuery.globalEval(( node.text || node.textContent ||
                                            node.innerHTML || "" ).replace(rcleanScript, ""));
                                }
                            }
                        }
                    }
    ...
    if (!support.opacity) {
        jQuery.cssHooks.opacity = {
            get: function (elem, computed) {
                ...
                return
                        ropacity.test((computed && elem.currentStyle ? elem.currentStyle.filter :
                                elem.style.filter) || "") ?
        ...
        hasClass: function (selector) {
            ...
            for (; i < l; i++) {
                if (this[i].nodeType === 1 &&
                        (" " + this[i].className + " ").replace(rclass, " ").indexOf(className) >=
                                0) {
                    return true;
                }
            }
                        ...
                        if (conv !== true) {
                            ...
                            if (conv && s[ "throws" ]) {
                                ...
                                try {
                                    ...
                                    return { state: "parsererror", error: conv ? e :
